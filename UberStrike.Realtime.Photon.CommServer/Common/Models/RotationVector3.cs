...
namespace UberStrike.Realtime.Common
{
    public struct RotationVector3
    {
        ...
        public RotationVector3(byte[] bytes, ref int idx)
        {
            ...
            int d2 = x; d2 |= y << 8;
            _vector.x = d2 % 360;
            _vector.y = CmuneMath.Clamp(d2 / 360, 0, 180) - 90;
            _vector.z = CmuneMath.Clamp(z, 0, 180) - 90;
        }
        ...
        {
            byte[] b = new byte[3];
            uint d1 = (uint)((v.x < 0 ? 360 + v.x % 360 : v.x % 360) + CmuneMath.Clamp(v.y + 90, 0, 180) * 360);
            b[0] = (byte)(d1 & 0xFF);
            b[1] = (byte)((d1 >> 8) & 0xFF);
            b[2] = (byte)(CmuneMath.Clamp(v.z + 90, 0, 180));
            bytes.AddRange(b);
        }
