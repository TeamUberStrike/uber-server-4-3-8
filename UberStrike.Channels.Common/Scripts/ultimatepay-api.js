...
UPay.prototype = {
        ...
        getXDJSON:function(url, opts/* specifically, success and error callbacks, mmkay? */){
            opts=opts||{};url=url||'';   //man, I wish we had an elvis operator in javascript
            var result = {};

            
            var requestObj = new jXHR();
            requestObj.onerror=function(msg,url){
                if(opts.error){
                    opts.error(msg,url);
                }
            };
            requestObj.onreadystatechange=function(data){
                if (requestObj.readyState === 4) {
                    result = data.source;   //meet my friend, Justin Case
                    if(opts.success){
                        opts.success(data);
                    }
              }
        ...
        displayUltimatePay: function(opts){
             ...
             */
            //new paypal flow enabling - stage 1
            //if(!(this.ieVersion>0) && isClick==true && opts.source!='LightboxLauncher' && window.ultimatePayParams && ultimatePayParams.paymentid && 'p3' == ultimatePayParams.paymentid.toLowerCase()){
            //    ultimatePayParams.paymentid = 'p2';        
            //}
          ...
          //new paypal flow enabling - stage 2
            //if 	(!(this.ieVersion>0) && isClick==true && window.ultimatePayParams && opts.source!='LightboxLauncher' && ultimatePayParams.paymentid && ultimatePayParams.paymentid.toLowerCase()=='p2' ){
            //    this.deepLinkPop();
            //    return;
            //}
        ...
        deepLinkPop:function(opts){
            var LBel = this.elFromHtml('<div id="DLFrameWrap" style="display:none;padding:10px;position:absolute;top: 50%;left: 50%;  margin-top:-212px;margin-left: -351px;"></div>');
            var LBif = this.elFromHtml('<iframe id="up_payframe" name="up_payframe" width="702" height="425" style="background-color:#ffffff;" scrolling="no" frameborder="0"></iframe>');
            LBel.appendChild(LBif);
            ...
            var baseUrl = (function(url){
                ...
                var prot = _split[0];
                var base = _split[1].split('/')[0];
                var result = prot + '//' + base ;
                return result;
            ...
            function sessCheck(){
                 var url =  baseUrl + '/app/api/live/?cb=?&method=GetServiceInfo&sn=UT13&_nc=' + Math.random();
                    ulp.getXDJSON(url, {
                        success:function(data){
                            
                            //alert(data.deeplinkToken);
                            var panUrl = baseUrl + "/app/order/flow/?PHPSESSID="+data.deeplinkToken;
                            ifSet(panUrl);
                        },error:function(){
                            alert('error');
                            alert(arguments[0])
            ...
            if(this.ieVersion>0){
//                var sessCheck = setTimeout(function(){
//                    
//                });
//                
//                
//                var cookieChecker = setInterval(function(){
//                    var panUrl = readCookie('_upPanelUrl');
//                    var panRefresh = readCookie('_upPanelRefresh');
//                    
//                    if(panUrl){
//                        
//                        LBif.src = ifUrl = unescape(panUrl);
//                        LBel.style.display='block';
//                        eraseCookie('_upPanelUrl');
//                        
//                    }
//                    if(panRefresh){
//                        LBif.src = ifUrl;
//                        LBel.style.display='block';
//                        eraseCookie('_upPanelRefresh');
...
//                },1000);
                ...
                pm.bind('doDLLBOpen', function(data){
                    ifSet(data.url);
                    
                });
                
                pm.bind('doDLLBRefresh', function(){
                    ifRefresh();
                    
                });
                
                pm.bind('doDLLBClose',function(){
                    Element.remove(LBel);
                    DLpopup.close();
            ...
            DLpopup = window.open(url,'pp','width=900,height=700,scrollbars=yes,resizable=yes,status=yes,menubar=yes,titlebar=yes,location=yes');
            if(this.ieVersion>0){
                
                var sessChecker = setInterval(function(){
                    if (!ifUrl){
                        
                        sessCheck();
                    }
                    else{
                        clearInterval(sessChecker);
                    }
        ...
        displayUltimatePayIframe: function(){
            ...
            if(this.upParams.display==='Lightbox' || this.upParams.display==='PlayStation3'){
        ...
        testForClick:function(){
            if(this.ieVersion>0){
                ...
                var isClick = false; 
                do{with(fn){
                    if(arguments[0] && arguments[0].type=='click'){
                        isClick = true; break;
                    }
...
Element.getStyle= function(element, style) {
    //element = $(element);
    style = style == 'float' ? 'cssFloat' : style;  //careful
...
Element.positionedOffset= function(element) {
    var valueT = 0, valueL = 0;
    do {
        valueT += element.offsetTop  || 0;
        valueL += element.offsetLeft || 0;
