...
if (typeof unityObject == "undefined")
	...
	ua = function () {
	    ...
	    var ua = {
	        ...
	        installUrl: "",  // Cmune unity install url (so that we can set it when 'ownInstallScreen' is being used)
	        installOnclick: "",
	        installNav: navigator
	    };
	    // get base url
	...
	onDomLoad = function () {
        ...
        function createMissingUnity(id, attributes, params, callback, broken) {
            ...
            if (re) {
                ...
                /* 
                * SHAUN: Edited here to use own custom images and sizes for No Unity screen
                * Ideally I can pass in a div that will just be shown
                * If I don't specify a div, then the code below can be used as a default
                */

                ua.installUrl = url; //Cmune
                ua.installOnclick = "unityObject.trackStatus('installing?type=" + type + "');";
                if (!ownInstallScreen) {  // Cmune
                    if (broken) {
        ...
        function createCSS(selector, declaration, media, newStyle) {
            ...
            if (ua.win && ua.ie && typeof styleSheet.addRule == "object") {
                ...
                if (styleSheet && typeof doc.createTextNode != "undefined") {
                    if (typeof styleSheet.appendChild === "function")
        ...
        function ieCleanup() {
            for (var i in listeners) {
                ...
                if (typeof listener.target != "undefined")
